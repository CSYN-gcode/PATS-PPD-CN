...
use App\Models\DbPpsPreshipment;
use App\Models\DbPpsPreshipTransaction;
...
class DbPpsPreShipmentController extends Controller
{
    ...
    public function getControlNumbers(Request $request){
        $all_control_no = DB::table('db_pps_preshipments AS pre_shipment')
                                ->select('pre_shipment.Packing_List_CtrlNo', 'pre_shipment.shipment_date')
                                ->when($request->control_number, function ($query) use ($request){
                                    return $query->where('pre_shipment.Packing_List_CtrlNo', $request->control_number);
                                })
    ...
    public function viewPreShipment(Request $request){
        $ProdRuncardData = DB::table('db_pps_preshipments AS pre_shipment')
                                ->select('*')
                                ->when($request->control_no, function ($query) use ($request){
                                    return $query->where('pre_shipment.Packing_List_CtrlNo', $request->control_no);
                                })
                                ->whereNull('pre_shipment.deleted_at')
                                ->get();
        ...
        ->addColumn('status', function ($row){
            ...
            switch($row->status){
                case 0: //Pending
                    $result .= '<center><span class="badge badge-pill badge-info">Pending</span></center>';
                    break;
                case 1: //Done
    ...
    public function viewSearchPoResult(Request $request){
        if(isset($request->pre_shipment_id)){
            $po_details = DB::table('db_pps_preship_transactions AS pre_shipmt_details')
                            ->select('*')
                            ->where('pre_shipmt_details.db_pps_preship_id', $request->pre_shipment_id)
                            ->whereNull('pre_shipmt_details.deleted_at')
                            ->get();
        }else{
            $po_details = DB::table('delivery_updates AS DeliveryUpdate')
            ->select('DeliveryUpdate.*', 'ProdRuncard.part_name', 'ProdRuncard.part_code')
            ->leftJoin('production_runcards AS ProdRuncard', 'DeliveryUpdate.runcard_id', '=', 'ProdRuncard.id')
        ...
        ->addColumn('row_master_carton_no', function($row){
            $data = '';
            if(isset($row->Master_CartonNo)){
                $data = $row->Master_CartonNo;
            }

            $result = "";
            $result .= "<center>";
                $result .= "<input type='number' class='master_carton_$row->id classToDisable' data-master_carton-id='$row->id' style='width: 100px; text-align: center;' id='masterCartonNoId' name='master_carton_no[]' value='".$data."'>";
            $result .= "</center>";
            return $result;
        })
        ->addColumn('item_no', function($row){
            ...
            $result .= "<center>";
            // if($row->status == 1 ){
                $result .= "<input type='number' class='item_$row->id classToDisable' data-item_no-id='$row->id' style='width: 100px; text-align: center;' id='itemNoId' name='item_no[]' value='".$data."'>";
            // }
            $result .= "</center>";
            return $result;
        })
        ...
        ->addColumn('row_package_qty', function($row){
            ...
            $result .= "<center>";
                ...
                $result .= "<input class='package_qty_$row->id classToDisable' data-package_qty-id='$row->id' type='text' style='width: 100px; text-align: center;' id='packageQtyId' name='package_qty[]'>";
            $result .= "</center>";
            return $result;
        })
        ...
        ->rawColumns(['checkbox', 'row_master_carton_no', 'lot_numbers', 'quantities', 'item_no', 'row_package_qty', 'row_remarks', 'remove_btn'])
        ->make(true);
    }
    ...
    public function viewPreShipmentDetails(Request $request){
            $PreShipDetails = DB::connection('mysql')->select("SELECT preship_details.*,
                                    ...
                                    FROM db_pps_preship_transactions AS preship_details
                                    LEFT JOIN users AS weighed_by_user ON preship_details.WeighedBy = weighed_by_user.id
                                    LEFT JOIN users AS packed_by_user ON preship_details.PackedBy = packed_by_user.id
                                    LEFT JOIN users AS checked_by_user ON preship_details.CheckedBy = checked_by_user.id
                                    -- WHERE preship_details.fkControlNo = '$request->pre_shipment_id'
    ...
    public function addPreShipmentData(Request $request){
        ...
        if ($validator->fails()) {
            ...
            try{
                if(!isset($request->pre_shipment_id)){
                    // Current year prefix (YY)
                    $currentYear = date('y');
                    // Current month (MM) for display
                    $currentMonth = date('m');

                    // Get latest pre-shipment record for current year
                    $latest = DbPpsPreshipment::whereNull('deleted_at')
                        ->where('Packing_List_CtrlNo', 'like', $currentYear . '%')
                        ->latest('id')
                        ->first();

                    // Determine counter
                    if ($latest) {
                        // Extract the numeric counter after the dash
                        $counter = (int) explode('-', $latest->Packing_List_CtrlNo)[1] + 1;
                    }else{
                        $counter = 1; // first record for this year
                    }

                    // Build control number in format YYMM-COUNTER
                    $control_number = $currentYear . $currentMonth . '-' . $counter;

                    // clark comment 01/21/2026
                    //Control Number Generation
                    // $lastest_id = DbPpsPreshipment::whereNull('deleted_at')->latest('id')->first();
                    // if($lastest_id == null){
                    //     $counter = 1;
                    // }else{
                    ...
                    $preshipment_id = DbPpsPreshipment::insertGetId([
                        // 'sales_cutoff'              => $request->sales_cutoff,
                        'date'                      => $request->date,
                        'Station'                   => $request->station,
                        'Packing_List_CtrlNo'       => $control_number,
                        'Shipment_Date'             => $request->shipment_date,
                        'Destination'               => $request->destination,
                        'Stamping'                  => $request->category == 'stamping' ? 1 : 0,
                        'grinding'                  => $request->category == 'grinding' ? 1 : 0,
                        'for_pps_cn_transfer'       => $request->category == 'for_pps_cn_transfer' ? 1 : 0,
                    ...
                    foreach($items as $item){
                        DbPpsPreshipTransaction::insert([
                            'db_pps_preship_id' => $preshipment_id,
                            'pkid'              => $item['id'],
                            'fkControlNo'       => $control_number,
                            'Master_CartonNo'   => $item['master_carton_no'],
                            'ItemNo'            => $item['item_no'],
                            'PONo'              => $item['po_no'],
                            'Partscode'         => $item['parts_code'],
                            'DeviceName'        => $item['device_name'],
                            'LotNo'             => $item['lot_no'],
                            'Qty'               => $item['qty'],
                            'PackageCategory'   => $item['package_category'] ?? null,
                            'PackageQty'        => $item['package_qty'],
                            'WeighedBy'         => $request->weighed_by,
                            'PackedBy'          => $request->packed_by,
                            'CheckedBy'         => $request->checked_by,
                            'Remarks'           => $item['remarks'] ?? null,
                            'LastUpdate'        => Auth::user()->id,
                            'Username'          => Auth::user()->id,
                            'created_at'        => now(),
                            'updated_at'        => now()
                        ]);
                    ...
                    //     DbPpsPreshipTransaction::insert([
                    //         'db_pps_preship_id'         => $preshipment_id,
                    //         'pkid'                      => $request->checkbox_id[$key],
                    //         'fkControlNo'               => $control_number,
                    //         'Master_CartonNo'           => $request->master_carton_no[$key],
                    //         'ItemNo'                    => $request->item_no[$key],
                    //         'PONo'                      => $request->po_number,
                    //         'Partscode'                 => $request->parts_code,
                    //         'DeviceName'                => $request->device_name,
                    //         'LotNo'                     => $request->lot_no[$key],
                    //         'Qty'                       => $request->quantity[$key],
                    //         'PackageCategory'           => $request->package_cat[$key],
                    //         'PackageQty'                => $request->package_qty[$key],
                    //         'WeighedBy'                 => $request->weighed_by,
                    //         'PackedBy'                  => $request->packed_by,
                    //         'CheckedBy'                 => $request->checked_by,
                    ...
                    DbPpsPreshipment::where('id', $request->pre_shipment_id)
                    ->update([
                        // 'sales_cutoff'              => $request->sales_cutoff,
                        'date'                      => $request->date,
                        'Station'                   => $request->station,
                    ...
                    // DELETE OLD Transaction upon UPDATE
                    DbPpsPreshipTransaction::where('db_pps_preship_id', $request->pre_shipment_id)->delete();

                    $items = json_decode($request->items, true);

                    foreach($items as $item){
                        DbPpsPreshipTransaction::insert([
                            'db_pps_preship_id' => $request->pre_shipment_id,
                            'pkid'              => $item['id'],
                            'fkControlNo'       => $request->control_no,
                            'Master_CartonNo'   => $item['master_carton_no'],
                            'ItemNo'            => $item['item_no'],
                            'PONo'              => $item['po_no'],
                            'Partscode'         => $item['parts_code'],
    ...
    //     $pre_shipment_data = DB::table('db_pps_preshipments AS pre_shipmt')
    //                         ->select('pre_shipmt.*', 'pre_shipmt_details.id AS preshipdetails_id',
    //                                                 'pre_shipmt_details.Master_CartonNo AS master_carton_no',
    //                                                 'pre_shipmt_details.ItemNo AS item_no',
    //                                                 'pre_shipmt_details.PONo AS po_no',
    //                                                 'pre_shipmt_details.Partscode AS parts_code',
    //                                                 'pre_shipmt_details.DeviceName AS device_name',
    //                                                 'pre_shipmt_details.LotNo AS lot_no',
    //                                                 'pre_shipmt_details.Qty AS qty',
    //                                                 'pre_shipmt_details.PackageQty AS package_qty',
    //                                                 'pre_shipmt_details.PackageCategory AS package_category',
    //                                                 'pre_shipmt_details.Remarks AS remarks'
    //                         )
    //                         ->leftJoin('db_pps_preship_transactions AS pre_shipmt_details', 'pre_shipmt.id', '=', 'pre_shipmt_details.db_pps_preship_id')
    //                         ->leftJoin('users AS weighed_by_user', 'pre_shipmt_details.WeighedBy', '=', 'weighed_by_user.id')
    //                         ->leftJoin('users AS packed_by_user', 'pre_shipmt_details.PackedBy', '=', 'packed_by_user.id')
    //                         ->leftJoin('users AS checked_by_user', 'pre_shipmt_details.CheckedBy', '=', 'checked_by_user.id')
    ...
    public function getPreShipmentData(Request $request){
        // Fetch pre-shipment header
        $preShipment = DB::table('db_pps_preshipments')
            ->where('id', $request->pre_shipment_id)
        ...
        if (!$preShipment) {
            return response()->json([
                'success' => false,
                'message' => 'Pre-shipment not found'
            ], 404);
        }

        // Fetch associated transactions
        $transactions = DB::table('db_pps_preship_transactions AS t')
            ->select(
                't.id AS preshipdetails_id',
                't.Master_CartonNo AS master_carton_no',
                't.ItemNo AS item_no',
                't.PONo AS po_no',
                't.Partscode AS parts_code',
                't.DeviceName AS device_name',
                't.LotNo AS lot_no',
                't.Qty AS qty',
                't.PackageQty AS package_qty',
                't.PackageCategory AS package_category',
                't.Remarks AS remarks',
                't.WeighedBy AS weighed_by',
                't.PackedBy AS packed_by',
                't.CheckedBy AS checked_by'
            )
            // ->leftJoin('users AS weighed_by_user', 't.WeighedBy', '=', 'weighed_by_user.id')
            // ->leftJoin('users AS packed_by_user', 't.PackedBy', '=', 'packed_by_user.id')
            // ->leftJoin('users AS checked_by_user', 't.CheckedBy', '=', 'checked_by_user.id')
            ->where('t.db_pps_preship_id', $preShipment->id)
    ...
    public function getPreShipmentForPrint(Request $request){
        $preship_data_to_print = DB::table('db_pps_preshipments AS pre_shipmt')
                            ->select('pre_shipmt.id AS preship_id',
                                        'pre_shipmt.Packing_List_CtrlNo AS preship_control_no',
                                        ...
                                        'pre_shipmt_details.*')
                            ->leftJoin('db_pps_preship_transactions AS pre_shipmt_details', 'pre_shipmt.Packing_List_CtrlNo', '=', 'pre_shipmt_details.fkControlNo')
                            ->leftJoin('users AS weighed_by_user', 'pre_shipmt_details.WeighedBy', '=', 'weighed_by_user.id')
                            ->leftJoin('users AS packed_by_user', 'pre_shipmt_details.PackedBy', '=', 'packed_by_user.id')
                            ->leftJoin('users AS checked_by_user', 'pre_shipmt_details.CheckedBy', '=', 'checked_by_user.id')
                            ->where('pre_shipmt.id', '=', $request->pre_shipment_id)
    ...
    public function getPreShipDetailsById(Request $request){
        $preship_details = DB::table('db_pps_preship_transactions AS preship_details')
                                ->select('preship_details.*')
                                ->where('preship_details.fkControlNo', $request->pre_shipment_id)//ATTENTION: CHANGE fkControlNo TO preshipment_pkid IF NEEDED
    ...
    public function addPreShipmentDetailsData(Request $request){
        ...
        try{
            if(!isset($request->pre_shipment_details_id)){
                // if(DbPpsPreshipTransaction::where('preshipment_pkid', $request->pre_shipment_id)->whereNull('deleted_at')->exists()){
                ...
                DbPpsPreshipTransaction::where('id', $request->pre_shipment_details_id)
    ...
    public function UpdateProdRuncardStatus(Request $request){
        date_default_timezone_set('Asia/Manila');
        // session_start();
        DbPpsPreshipment::where('id', $request->runcard_id)
                    ->update([
                        'status'              => 1,
                        'last_updated_by'     => Auth::user()->id,
                        'updated_at'          => date('Y-m-d H:i:s'),
        ...
        DbPpsPreshipTransaction::where('prod_runcards_id', $request->runcard_id)
                    ->update([
                        'status'              => 1,
                        'last_updated_by'     => Auth::user()->id,
                        'updated_at'          => date('Y-m-d H:i:s'),
    ...
    public function SubmitProdRuncard(Request $request){
        date_default_timezone_set('Asia/Manila');
        // session_start();
        DbPpsPreshipment::where('id', $request->cnfrm_assy_id)
                    ->update([
                        'status'              => 3,
                        'last_updated_by'     => Auth::user()->id,
                        'updated_at'          => date('Y-m-d H:i:s'),
        ...
        DbPpsPreshipTransaction::where('prod_runcards_id', $request->cnfrm_assy_id)
                    ->update([
                        'status'              => 3,
                        'last_updated_by'     => Auth::user()->id,
                        'updated_at'          => date('Y-m-d H:i:s'),
    ...
    public function CheckExistingStations(Request $request){
        $prod_runcard_details = DbPpsPreshipment::with('device_details.material_process.station_details.stations', 'runcard_station')->where('id', $request->runcard_id)->whereNull('deleted_at')->first();
        ...
        if($previous_station_step < $count_of_steps){
            ...
            $output_qty = DbPpsPreshipTransaction::whereNull('deleted_at')
    ...
    public function getPreshipmentQrcode (Request $request){
        $data = DbPpsPreshipment::select('db_pps_preshipments.id',
                                            'details.po_no',
                                            'db_pps_preshipments.shipment_date',
                                            'details.qty',
                                            'details.device_name',
                                            'details.parts_code',
                                            'details.packed_by',
                                            'details.lot_no',
                                            'db_pps_preshipments.status AS preshipment_status',
                                            DB::raw("CONCAT( firstname, ' ', lastname) AS operator_name"))
                                    ->leftJoin('db_pps_preship_transactions AS details', function($join){
                                        $join->on('details.preshipment_pkid', '=' ,'db_pps_preshipments.id');
                                    })
                                    ->leftJoin('users', function($join){
                                        $join->on('users.id', '=', 'db_pps_preshipments.last_updated_by');
                                    })
