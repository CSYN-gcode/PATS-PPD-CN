...
class DeviceController extends Controller
{
        ...
        return DataTables::of($devices)
            ...
            ->addColumn('action', function($device){
                ...
                if($device->status == 1){
                    ...
                    // $result .= '<button class="dropdown-item aShowDeviceDevProc" device-id="' . $device->id . '" device-name="' . $device->name . '" type="button" style="padding: 1px 1px; text-align: center;">Show Devices Process</button>';
    ...
    public function get_device_by_id(Request $request){
        $device = Devices::where('id', $request->device_id)
        ->where('status', 1)
        ->first();
    ...
    public function change_device_stat(Request $request){
        DB::beginTransaction();
        try{
            Devices::where('id', $request->device_id)
            ->update([
