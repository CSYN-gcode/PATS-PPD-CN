...
use App\Models\ProductionRuncardCavity;
...
class ProductionRuncardController extends Controller
{
    ...
    public function getCavityCount(Request $request){
        $cavityCount = DB::table('devices')
                        ->select("cavity_count")
    ...
    public function ValidateMatLotNumber(Request $request){
        $whs_material_name = DB::connection('mysql_rapid_pps')
                            // ->select(' SELECT *
                            ->select(' SELECT whs.MaterialType AS mat_name, whs.PartNumber AS mat_code, whs_transaction.Lot_number AS lot_no, whs_class.id AS class_id
                                    ...
                                    -- WHERE whs_transaction.Lot_number = "'.$request->mat_lot_number.'" AND whs.MaterialType = "'.$request->material_name.'"
                                    WHERE whs_transaction.Lot_number = "'.$request->mat_lot_number.'"
                                    LIMIT 1
                                ');

        // return $request->device_name;
        // return $whs_material_name[0]->mat_code;
        ...
        if($whs_material_name != []){
                ...
                $matrix_details = DB::connection('mysql')->table('material_processes')
                                ...
                                ->where('devices.name', $request->device_name)
                                ->where('material_process_materials.material_code', $whs_material_name[0]->mat_code)
                                ->get();
    ...
    public function getProdRuncardData(Request $request){
        ...
        return response()->json(['runcard_data' => $prod_runcard_data, 'mode_of_defect_data' => $mode_of_defect_data, 'cavity_count_data' => $cavity_count_data, 'cav_data_mode' => $cav_data_mode]);
    }
        ...
        // }else {
            ...
            try{
                if(!isset($request->frmstations_runcard_station_id)){
                    if(ProductionRuncardStation::where('prod_runcards_id', $request->frmstations_runcard_id)
                                                ->where('station', $request->runcard_station)
                                                ->where('sub_station', $request->runcard_sub_station)
                                                ->whereNull('deleted_at')
                                                ->exists())
                    {
    ...
    public function GetMatrixDataByDevice(Request $request){
        ...
        $matrix_data = Devices::with(['material_process.material_details', 'material_process.station_details.stations', 'material_process' => function ($q) {
                                        $q->orderBy('step', 'asc'); // 👈 order material_process by step
                                    },])
        ->when($request->device_name, function ($query) use ($request){
